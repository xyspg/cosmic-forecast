...
export function FlareTicker() {
  ...
  return (
    <div className="relative flex h-[26px] items-center overflow-hidden border-b border-black bg-[#0a0a0a] text-[#d4cfbf]">
      <div className="bg-amber relative z-[2] flex h-full shrink-0 items-center px-[10px] font-mono text-[10px] font-semibold tracking-[0.18em] text-black">
        ◈ SOLAR OBS · LIVE
      </div>
      <div
        className="flex gap-7 pl-5 font-mono text-[11px] whitespace-nowrap"
        style={{ animation: "bureau-ticker-scroll 90s linear infinite" }}
      >
        {row.map((it, i) => (
          <div key={`${it.t}-${i}`} className="flex items-baseline gap-[10px]">
