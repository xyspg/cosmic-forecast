...
- **Vite 8** with `@cloudflare/vite-plugin` — single dev server runs the React SPA *and* the Hono Worker, with KV bindings via Miniflare
...
`env` (KV, OPENAI_*) is **passed explicitly** through `c.env` — no `getCloudflareContext` magic, no `process.env`. Library functions like `generateExplanation` take `env` as a parameter.
